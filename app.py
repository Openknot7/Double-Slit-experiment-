...
# Prepare data for Altair (fixes 'Infinite extent' warnings by using explicit DataFrame)
hist_bins = np.linspace(-LY/2, LY/2, 41)
bin_centers = 0.5 * (hist_bins[1:] + hist_bins[:-1])

if len(st.session_state.hits) > 0:
    counts, _ = np.histogram(st.session_state.hits, bins=hist_bins)
else:
    counts = np.zeros(40)
...
hist_spot.altair_chart(chart, use_container_width=True)
